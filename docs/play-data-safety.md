...
**What the code actually does on offboarding** (`accounts/models.py`, `UserProfile.save()`): setting a `contract_end_date` on an employee's profile automatically flips `User.is_active = False` and force-revokes their login — their DRF token is deleted and every active Django session is killed immediately (`_force_logout()`).

**Confirmed with the app owner** (no longer a guess): deactivation is the whole process — data is retained indefinitely afterward, not purged after a fixed period, and there is currently no path for an employee to explicitly request deletion rather than being offboarded. This is now stated plainly in the published privacy policy (see below).

**Whether Play's account-deletion *requirement* even applies here — checked against current policy, not assumed**: Play Console's account-deletion policy (support.google.com/googleplay/android-developer/answer/13327111, checked live) only requires in-app + web deletion for accounts a user can create "directly in the app" or via an "account creation flow outside of the app" that the app directs them to; it explicitly excludes "accounts that are created and operated offline." `accounts/urls.py:30` has the self-registration route commented out, and there's no in-app or app-directed signup flow anywhere in this codebase — every Lilium Pharma account is provisioned entirely by an administrator via Django admin, outside the app. That puts this app's accounts in the "created and operated offline" category, so the in-app/web deletion-flow *requirement* likely doesn't apply — this is still your judgment call to make in the Play Console form itself (not something a doc can decide for you), but it's a call backed by both the code and Google's current published policy language, not a guess.
